...
class TestEgg(ut.TestCase):
    ...
    def test__prob(self):
        ...
        with mk.patch.object(stats.norm, 'cdf', autospec=True) as mkCDF:
            # Test not less
            self.assertEqual(self.Egg._prob(age), 0)
            ...
            # Test less
            self.assertEqual(self.Egg._prob(age),
                             mkCDF.return_value)
            self.assertEqual(mkCDF.call_args_list,
                             [mk.call(age,
    ...
    def test___call__(self):
        ...
        with mk.patch.object(model.Egg, '_prob', autospec=True) as mkProb:
            with mk.patch.object(rnd, 'random') as mkRND:
                mkRND.return_value.__le__.side_effect = [True, False]

                # Return True
                self.assertTrue(self.Egg(mass, age, genotype))
                ...
                self.assertEqual(mkProb.call_args_list,
                                 [mk.call(self.Egg, age)])

                mkRND.reset_mock()
                mkProb.reset_mock()
                # Return False
                self.assertFalse(self.Egg(mass, age, genotype))
                self.assertEqual(mkRND.return_value.__le__.call_args_list,
                                 [mk.call(mkProb.return_value)])
                self.assertEqual(mkRND.call_args_list,
                                 [mk.call()])
...
class TestPupa(ut.TestCase):
    ...
    def test__prob(self):
        ...
        with mk.patch.object(stats.norm, 'cdf', autospec=True) as mkCDF:
            # Test not less
            self.assertEqual(self.Pupa._prob(age), 0)
            self.assertEqual(mkCDF.call_args_list, [])
            self.assertEqual(self.minimum.__le__.call_args_list,
                             [mk.call(age)])
            ...
            self.minimum.reset_mock()
            # Test less
            self.assertEqual(self.Pupa._prob(age),
                             mkCDF.return_value)
            self.assertEqual(mkCDF.call_args_list,
                             [mk.call(age,
                                      loc=self.mu, scale=self.sigma)])
    ...
    def test___call__(self):
        ...
        with mk.patch.object(model.Pupa, '_prob', autospec=True) as mkProb:
            with mk.patch.object(rnd, 'random') as mkRND:
                mkRND.return_value.__le__.side_effect = [True, False]

                # Return True
                self.assertTrue(self.Pupa(mass, age, genotype))
                self.assertEqual(mkRND.return_value.__le__.call_args_list,
                                 [mk.call(mkProb.return_value)])
                self.assertEqual(mkRND.call_args_list,
                                 [mk.call()])
                self.assertEqual(mkProb.call_args_list,
                                 [mk.call(self.Pupa, age)])

                mkRND.reset_mock()
                mkProb.reset_mock()
                # Return False
                self.assertFalse(self.Pupa(mass, age, genotype))
                self.assertEqual(mkRND.return_value.__le__.call_args_list,
                                 [mk.call(mkProb.return_value)])
                self.assertEqual(mkRND.call_args_list,
                                 [mk.call()])
