...
#
# save_file = 'susceptible_sim_0_bt_error_plots.html'
# plt.output_file(save_file)
#
#
# sim = error.ProcessData(runs.runs[1]).plot()
#
# layout = lay.column(
#     sim[runs.tables[0]][runs.columns[2]],
#     sim[runs.tables[1]][runs.columns[2]],
#     sim[runs.tables[2]][runs.columns[2]],
#     sim[runs.tables[3]][runs.columns[2]],
# )
# plt.show(layout)
import bokeh.plotting as plt
...
sim = error.ProcessData(runs.runs[1]).series()

egg    = sim[runs.tables[0]][runs.columns[2]]
larva  = sim[runs.tables[1]][runs.columns[2]]
pupa   = sim[runs.tables[2]][runs.columns[2]]
female = sim[runs.tables[3]][runs.columns[2]]

reg_plot = plt.figure(plot_height=plot_height,
                      plot_width=plot_width)
reg_plot.title.text = 'Susceptible Error'
